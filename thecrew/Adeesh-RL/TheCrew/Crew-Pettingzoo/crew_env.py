...
class CustomCrewEnvironment(AECEnv):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.deck = [(color, rank) for color in range(self.num_colors) for rank in range(self.num_ranks)] + [(4, rank) for rank in range(self.num_rockets)]
        self.hands = self.distribute_cards()
        self.target_missions = [self.assign_target() for _ in range(self.total_target_missions)]  # Reassign target cards to players
        self.commander = 0
    ...
    def get_legal_moves(self, agent, observation):
        ...
        action_mask = np.zeros(self.observation_space['action_mask'].shape, dtype=int)
